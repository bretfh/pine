...
repl:
	$(GUIX) sh -c '$(ENV) sbcl --eval "(require :asdf)" --eval "(asdf:load-system :pine)"'
...
bin:
	$(GUIX) sh -c '$(ENV) sbcl --non-interactive --eval "(require :asdf)" --eval "(asdf:load-system :pine/wayland)" --eval "(sb-ext:save-lisp-and-die \".pine.bin\" :toplevel (function pine::cli) :executable t)" && printf '\''#!/bin/sh\nexport GUIX_ENVIRONMENT="%s"\nexport LD_LIBRARY_PATH="%s/lib:$$LD_LIBRARY_PATH"\nexec "$$(dirname "$$0")/.pine.bin" "$$@"\n'\'' "$$GUIX_ENVIRONMENT" "$$GUIX_ENVIRONMENT" > pine && chmod +x pine'
	@echo "built ./pine"

# the headless substrate: editor and desktop apps attach to it over remoting.
daemon:
	$(GUIX) sh -c '$(ENV) sbcl --eval "(require :asdf)" --eval "(asdf:load-system :pine)" --eval "(pine:run-daemon)"'

# the editor app: a separate process, attaches to the daemon.
editor:
	$(GUIX) sh -c '$(ENV) $(PRELOAD) sbcl --eval "(require :asdf)" --eval "(asdf:load-system :pine/gtk)" --eval "(pine.gtk:run-editor)"'

# the desktop app: layer-shell bar/panels, a separate process, attaches to the daemon.
desktop:
	$(GUIX) sh -c '$(ENV) $(PRELOAD) sbcl --eval "(require :asdf)" --eval "(asdf:load-system :pine/gtk)" --eval "(pine.desktop-app:run-desktop-app)"'
...
dev:
	$(GUIX) sh -c '$(ENV) \
	  ...
	  sbcl --non-interactive --eval "(require :asdf)" --eval "(asdf:load-system :pine)" --eval "(pine:run-daemon)" >/tmp/pine-daemon.log 2>&1 & \
	  DPID=$$!; \
	  for i in $$(seq 1 90); do grep -q "daemon up" /tmp/pine-daemon.log && break; sleep 1; done; \
	  $(PRELOAD) sbcl --non-interactive --eval "(require :asdf)" --eval "(asdf:load-system :pine/gtk)" --eval "(pine.desktop-app:run-desktop-app)" >/tmp/pine-desktop.log 2>&1 & \
	  WPID=$$!; \
	  $(PRELOAD) sbcl --eval "(require :asdf)" --eval "(asdf:load-system :pine/gtk)" --eval "(pine.gtk:run-editor)"; \
	  kill $$DPID $$WPID 2>/dev/null'

check:
	$(GUIX) sh -c '$(ENV) sbcl --non-interactive --eval "(require :asdf)" --eval "(asdf:load-system :pine/gtk)" --eval "(princ :loaded)" --eval "(terpri)"'
...
wl-desktop:
	$(GUIX) sh -c '$(ENV) sbcl --eval "(require :asdf)" --eval "(asdf:load-system :pine/wayland)" --eval "(pine.wayland:run-desktop)"'
...
wl-editor:
	$(GUIX) sh -c '$(ENV) sbcl --eval "(require :asdf)" --eval "(asdf:load-system :pine/wayland)" --eval "(pine.wl-editor:run-editor)"'

# print each highlighted token of a source file and its face: make hl FILE=x.lisp
hl:
	$(GUIX) sh -c '$(ENV) sbcl --non-interactive --eval "(require :asdf)" --eval "(asdf:load-system :pine)" --eval "(pine.ts:hl-dump-file \"$(FILE)\")"'

# render the editor frame to a PNG, headless (no window): make shot
shot:
	$(GUIX) sh -c '$(ENV) sbcl --non-interactive --eval "(require :asdf)" --eval "(asdf:load-system :pine/gtk)" --eval "(pine.gtk::shot)" --eval "(sb-ext:exit)"'

# render every desktop panel through the GTK-free cairo backend to PNGs in /tmp,
# headless (no window, no daemon): make cairo-shot
cairo-shot:
	$(GUIX) sh -c '$(ENV) sbcl --non-interactive --eval "(require :asdf)" --eval "(asdf:load-system :pine/cairo)" --eval "(princ (pine.cairo:shot))" --eval "(terpri)" --eval "(sb-ext:exit)"'
