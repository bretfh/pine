...
(test a-window-is-a-place-under-the-compositor
  (%managed)
  (is (equal "browser" (fs:contents (fs:at "/wm/windows/2/title"))))
  (is (fs:contents (fs:at "/wm/windows/2/focused")))
  (is (equal '("1" "2" "3") (fs:contents (fs:at "/wm/windows")))))

(test what-a-window-says-about-itself-is-a-path-each
  (%managed)
  (is (equal "browser" (fs:contents (fs:at "/wm/windows/2/title"))))
  (is (equal "chrome" (fs:contents (fs:at "/wm/windows/2/app"))))
  (is (fs:contents (fs:at "/wm/windows/2/focused")))
  (is (null (fs:contents (fs:at "/wm/windows/1/focused")))))

(test an-output-is-a-place-and-says-what-the-bars-left
  (%managed)
  (is (equal '("eDP-1") (fs:contents (fs:at "/wm/outputs"))))
  (is (equal '(1280 720) (fs:contents (fs:at "/wm/outputs/eDP-1/size"))))
  (is (equal '(0 0) (fs:contents (fs:at "/wm/outputs/eDP-1/position"))))
  (is (equal '(0 0 1280 720) (fs:contents (fs:at "/wm/outputs/eDP-1/area"))))
  (setf (fs:contents (fs:at "/wm/said"))
        (list :windows nil
              :outputs '((:name "eDP-1" :position (0 0) :size (1280 720)
                          :area (64 0 1216 720)))
              :focused nil))
  (is (equal '(64 0 1216 720) (fs:contents (fs:at "/wm/outputs/eDP-1/area")))
...
(test a-window-manager-is-something-that-writes-the-placement
  ...
  (is (equal '((1 0 0 640 720) (2 640 0 640 720))
             (fs:contents (fs:at "/wm/placement"))))
...
(test what-pine-wants-of-the-compositor-is-taken-once
  ...
  (let ((wants (fs:contents (fs:at "/wm/wants"))))
    (is (= 2 (length wants)))
    (is (eq :close (first (second wants)))))
  (is (null (fs:contents (fs:at "/wm/wants")))
      "and the next one to look finds nothing"))

(test a-verb-is-a-place
  (%managed)
  (setf (fs:contents (fs:at "/wm/close")) t)
  (is (equal '((:close)) (fs:contents (fs:at "/wm/wants")))))

(test taking-a-window-off-the-screen-is-writing-that-it-is-off
  (%managed)
  (fs:contents (fs:at "/wm/wants"))
  (setf (fs:contents (fs:at "/wm/windows/2/hidden")) t)
  (is (equal '((:hide "2")) (fs:contents (fs:at "/wm/wants"))))
  (setf (fs:contents (fs:at "/wm/windows/2/hidden")) nil)
  (is (equal '((:show "2")) (fs:contents (fs:at "/wm/wants")))))

(test the-keyboard-goes-where-the-focused-place-says
  (%managed)
  (fs:contents (fs:at "/wm/wants"))
  (setf (fs:contents (fs:at "/wm/focused")) "3")
...
it, and the mode that answers is the window manager's."
  ...
  (fs:contents (fs:at "/wm/wants"))
  (setf (fs:contents (fs:at "/wm/key")) "s-q")
  (is (null (fs:contents (fs:at "/wm/wants")))
      "a chord nothing bound does nothing")
  (setf (fs:contents (fs:at "/wm/key")) "s-c")
  (is (equal '((:close)) (fs:contents (fs:at "/wm/wants")))
...
(test a-chord-of-the-window-managers-is-not-the-editors
  ...
  (setf (fs:contents (fs:at "/wm/key")) "s-x")
  (is (equal "s-x" (fs:contents (fs:at "/wm/key")))
  ...
  (setf (ui:pending) nil)
  (setf (fs:contents (fs:at "/wm/key")) "s-r")
  (is (equal "" (fs:contents (fs:at "/wm/key")))
...
write, and most of them are not written beside the root one."
  ...
  (let ((n (fs:at "/mode/wm/keys")))
    (is (not (null n)) "the keymap is a place")
    (is (equal "wm-close-window" (d:lookup (fs:contents n) "s-c"))
        "as a map, which crosses a wire like anything else"))
  (let ((n (fs:at "/mode/prompt/keymap")))
    (is (not (null n)))
    (is (d:lookup (fs:contents n) "RET")
