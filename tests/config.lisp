...
(test what-the-example-declared-is-there
  ...
  (is (not (null (mode:binding (make-instance 'mode:text) "C-c h")))
      "a chord it bound")
  (is (not (null (fs:at "/ui/surface" "ticker"))) "a surface it declared")
  (is (not (null (fs:at "/ui/surface" "sound"))))
...
(test a-surface-a-config-declared-crosses-the-wire
  (editing)
  (pine:load-config (%example))
  (let ((form (fs:contents (fs:at "/ui/surface/ticker/wire"))))
    (is (not (null form)))
