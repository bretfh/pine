...
(test the-jobs-buffer-lists-the-local-evaluations
  (with-fixture substrate ()
...
(test persisted-variables-and-refs-reseed-after-a-restart
  (with-fixture substrate ()
    ...
    (is (= 5 (pine.state.store:store '(:var :persist-probe))))
    ...
    (is (= 9 (pine.state.store:store '(:ref :persist-ref-probe))))
...
(test a-kill-reaches-the-store
  (with-fixture substrate ()
    ...
    (is (member "persist-kill-probe" (pine.state.store:store :kill-ring)
                :test #'equal))))
...
(test find-file-records-a-recent-and-restores-the-saved-place
  (with-fixture substrate ()
    (let ((path "/tmp/pine-place-probe.txt"))
      ...
      (sleep 0.15)
      (is (member path (pine.state.store:store-items :recent-files) :test #'equal))
      (let ((buf (pine.editor.frame::buffer "pine-place-probe.txt")))
        ...
        (sleep 0.1)
        (is (equal '(2 1) (pine.state.store:store (list :place path)))))
...
(test the-world-round-trips-the-splits-the-open-file-and-the-focus
  (with-fixture substrate ()
    ...
    (let ((path "/tmp/pine-world-probe.txt"))
      ...
      (let ((arrangement (pine.state.store:store '(:world :arrangement)))
            (buffers (pine.state.store:store '(:world :buffers))))
        (is (eq :column (first (getf arrangement :tree))))
        (is (equal "pine-world-probe.txt" (getf arrangement :current)))
        (is (assoc path buffers :test #'equal))
        (pine.editor.frame::kill-buffer "pine-world-probe.txt")
        (sleep 0.1)
        (pine.editor.session::%seed-editor-tree *client* :world nil)
        (pine.ui.render:relayout)
        (is (= 1 (length (pine.editor.window::%editor-leaves *client*))))
        (setf (pine.state.store:store '(:world :arrangement)) arrangement)
        (pine.state.world:restore-world :buffers)
        (sleep 0.15)
...
(test scratch-text-rides-its-own-world-contributor
  (with-fixture substrate ()
    ...
    (let ((scratch (pine.editor.frame::buffer "scratch")))
      ...
      (sleep 0.1)
      (pine.state.world:restore-world :scratch)
      (sleep 0.1)
