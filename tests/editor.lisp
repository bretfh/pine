  ...
  (let ((chosen :none))
    ...
    (is (equal '("forward-word")
               (mapcar #'pine.editor:candidate-string
                       (pine.editor.frame:filtered (pine.editor::completion)))))
    (let ((rows (pine.editor.frame:popup-rows (pine.editor::completion))))
      (is (not (null rows)))
      (is (search "forward-word" (car (first rows))))
      (is (equal (multiple-value-bind (r g b)
                     (pine.text.buffer:hex-rgb (pine.text.buffer:color :bg-active))
                   (list r g b))
...
(test (prompt-abort :depends-on nested-prompt)
  ...
  (let ((chosen :none))
    ...
    (is (not (eq (pine.editor.frame:current-buffer *client*)
                 (pine.editor.frame:minibuffer-buffer *client*))))
    (is (null (pine.editor.frame:popup-rows (pine.editor::completion))))
...
(test (layout-render :depends-on jobs-buffer)
  ...
  (multiple-value-bind (rows root)
    ...
    (let ((run (rest (first (cdr (first rows))))))
      (is (equal (multiple-value-bind (r g b)
                     (pine.text.buffer:hex-rgb (pine.text.buffer:color :fg))
                   (list (list r g b) 1))
    ...
    (let ((arr (pine.state.store:store '(:world :arrangement)))
      ...
      (let* ((leaves (pine.editor::%editor-leaves *client*))
             (names (mapcar (lambda (l)
                              (pine.text.buffer:window-name (pine.ui.node:window-of l)))
                            leaves)))
        (is (= 3 (length leaves)))
        (is (= 2 (count "pine-world-probe.txt" names :test #'equal)))
        (is (equal "pine-world-probe.txt"
                   (pine.text.buffer:window-name (pine.editor.frame:focused-window *client*))))
        (is (string= "alpha"
...
modeline follow, layout-buffer embeds."
  (let* ((f (pine.editor.frame:frame *client*))
    ...
    (setf (pine.text.buffer:frame-cols f) 40 (pine.text.buffer:frame-rows f) 12)
    ...
    (let* ((wa (pine.editor:editor-window-node "scratch" :expand 1))
      ...
      (setf (pine.editor.frame:current-buffer *client*)
            (pine.text.buffer:buffer-ref (pine.ui.node:window-of wa)))
      (sleep 0.2)
    ...
    (is (= 1 (length (pine.editor::%editor-leaves *client*))))
    (is (eq scratch (pine.text.buffer:buffer-ref (pine.editor.frame:focused-window *client*))))
...
(test rules-wire
  ...
  (let* ((got nil)
    ...
    (unwind-protect
         (progn
           (pine.text.buffer:add-rules '((".wire-probe" (:opacity "0.5"))))
           (sleep 0.2)
