...
activation resolves and advances; the eval target follows the fault."
  ...
  (is (equal "beta" pine.editor::*eval-target*))
  (let ((txt (btext (pine.buffer:buffer "*debugger*"))))
    (is (search "alpha" txt))
    (is (search "beta" txt))
    (is (search "[ABORT]" txt)))
  ...
  (is (equal "alpha" pine.editor::*eval-target*))
  ...
  (is (eq :local pine.editor::*eval-target*)))
...
(test (jobs-buffer :depends-on one-eval-path)
  ...
  (let ((txt (btext (pine.buffer:buffer "*jobs*"))))
    (is (search "Jobs" txt))
    (is (search "local" txt))))
...
fires, reproject takes a new width."
  (let ((fired nil)
        (buf (pine.buffer:make-buffer "layout-probe")))
...
major modes, variables, css rules, tool buffers, prompts."
  ...
  (is (string= (format nil "tool~%> a~%  b")
               (btext (pine.buffer:buffer "*user-tool*"))))
