...
(test choosing-and-completing-fills-the-answer
  (with-prompt ()
    (pine.edit.prompt:ask "M-x " :category :command)
    (typed "split-window")
    (pine.edit.prompt:complete!)
    (is (equal "split-window-" (pine.edit.prompt:said))
        "tab fills in as much as every match agrees on, and no more")
    (typed "b")
    (pine.edit.prompt:complete!)
    (is (equal "split-window-below" (pine.edit.prompt:said))
        "and the whole of it once one match is left")
    (pine.edit.prompt:cancel!)
    (pine.edit.prompt:ask "M-x " :category :command :must-match t)
    (typed "window")
    (is (< 1 (length (pine.edit.prompt:matching))) "several candidates match")
    (let ((in-front (pine.edit.prompt:name-of
                     (first (pine.edit.prompt:matching)))))
      (pine.edit.prompt:choose! 1)
      (is (eql 1 (pine.edit.prompt:chosen (pine.edit.prompt:asking))))
      (is (not (equal in-front (pine.edit.prompt:answer!)))
          "what is chosen is what answering takes"))))
...
(test every-word-must-be-in-it-and-the-order-is-yours
  (with-prompt ()
    (pine.edit.prompt:ask "M-x " :category :command)
    (typed "win split")
    (let ((found (named (pine.edit.prompt:matching))))
      (is (member "split-window-below" found :test #'equal)
          "two words in either order, which is how a name is half remembered")
      (is (null (member "find-file" found :test #'equal))))
    (pine.edit.prompt:cancel!)))

(test what-was-answered-here-before-comes-back
  (with-prompt ()
    (pine.edit.prompt:ask "M-x " :category :command :history :probe)
    (typed "list-buffers")
    (pine.edit.prompt:answer!)
    (pine.edit.prompt:ask "M-x " :category :command :history :probe)
    (is (equal "" (pine.edit.prompt:said)))
    (pine.edit.prompt:walk-history 1)
    (is (equal "list-buffers" (pine.edit.prompt:said))
        "M-p walks back through what this question was answered with")
    (pine.edit.prompt:walk-history -1)
    (is (equal "" (pine.edit.prompt:said)) "and M-n comes back to what was typed")
    (pine.edit.prompt:cancel!)))

(test a-file-prompt-opens-where-the-buffer-is
  (with-prompt ()
    (pine.edit.prompt:ask "Find file: " :category :file)
    (is (equal (namestring (uiop:getcwd)) (pine.edit.prompt:said))
        "a file question starts from somewhere rather than from nothing")
    (is-true (member "src/" (named (pine.edit.prompt:matching)) :test #'equal)
             "and lists what is there, directories first")
    (pine.edit.prompt:cancel!)))

(test a-directory-is-descended-into-rather-than-answered
  (with-prompt ()
    (let ((got nil))
      (pine.edit.prompt:ask "Find file: " :category :file
                            :then (lambda (said) (setf got said)))
      (pine.edit.prompt:cancel!)
      (pine.edit.prompt:ask "Find file: " :category :file :initial "/tmp"
                            :then (lambda (said) (setf got said)))
      (is-true (pine.edit.prompt:descends-p (pine.edit.prompt:asking)))
      (press "RET")
      (is (null got) "RET on a directory went into it")
      (is (equal "/tmp/" (pine.edit.prompt:said)))
      (pine.edit.prompt:cancel!))))

(test the-candidates-float-above-the-echo-line
  (with-prompt ()
    (press "M-x")
    (typed "split")
    (let* ((tree (pine.edit.render:frame-tree :cols 60 :rows 12))
           (echo (%classed tree "echo")))
      (is (< 1 (length (pine.ui.node:view-rows echo)))
          "the candidates are rows of the echo node, not a block of their own")
      (is (eql 1 (pine.ui.node:view-base echo))
          "so they are painted over the buffer rather than pushing it up")
      (is (search "split-window-below"
                  (princ-to-string (pine.ui.node:view-rows echo)))))
    (pine.edit.prompt:cancel!)))
