...
(test undo-is-cheap-because-the-seqs-share
  (with-editor ()
    ...
    (let ((before (pine.data:held (pine.edit.buffer:lines (b)))))
      (pine.edit.buffer:insert! (b) "x")
      (let ((after (pine.data:held (pine.edit.buffer:lines (b)))))
        (is (eq (pine.data:at before 0) (pine.data:at after 0)))
        (pine.edit.buffer:undo! (b))
        (is (eq before (pine.data:held (pine.edit.buffer:lines (b))))
...
(test a-long-line-scrolls-sideways-under-the-window
  (with-editor (:text "0123456789")
    (let ((w (pine.edit.window:focused))
          (b (b)))
      (setf (pine.fs.node:contents b)
            (format nil "~{~d~}" (loop :for i :below 200 :collect (mod i 10))))
      (setf (pine.edit.window:width-of w) 20
            (pine.edit.window:height-of w) 4)
      (pine.edit.buffer:goto! b 0 0)
      (pine.edit.render:scroll-to-point w)
      (is (= 0 (pine.edit.window:hscroll-of w)))
      (pine.edit.buffer:goto! b 0 120)
      (pine.edit.render:scroll-to-point w)
      (is (= 101 (pine.edit.window:hscroll-of w))
          "point past the right edge brings the line under it")
      (let ((view (pine.edit.render:buffer-tree w)))
        (is (= 19 (pine.ui.node:view-ccol view))
            "and the caret sits at the edge it scrolled to")
        (is (equal (subseq (car (first (pine.ui.node:view-rows view))) 0 4) "1234")
            "the row starts at the column it scrolled to"))
      (pine.edit.buffer:goto! b 0 0)
      (pine.edit.render:scroll-to-point w)
      (is (= 0 (pine.edit.window:hscroll-of w))
