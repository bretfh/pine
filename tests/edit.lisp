...
(test the-prompt-is-a-mode-and-narrows-as-you-type
  ...
  (is (prompt:askingp))
  (is (typep (text:mode-of (text:current)) 'emode:prompt))
...
(test a-listing-row-stands-for-a-thing
  ...
  (is (typep (listing:place) 'text:document))
  (is (typep (text:mode-of (text:current)) 'emode:listing)))
...
(test what-is-at-point-is-a-symbol-the-mode-knows
  (let ((doc (editing)))
    ...
    (text:goto doc 0 2)
    (is (search "car" (or (pine/edit/eval:arglist (text:mode-of doc) doc) "")))
