...
(test a-path-is-read-as-sugar-and-means-a-list-of-segments
  (let ((p (pine/path/path:parse "/buffer/scratch/text")))
    (is (equal "/buffer/scratch/text" (pine/path/path:text p)))
    (is (equal "text" (pine/path/path:leaf p)))
    (is (equal "/buffer/scratch" (pine/path/path:text (pine/path/path:parent p))))
    (is-false (pine/path/path:patternp p))))

(test a-pattern-binds-what-it-matched
  (let ((p (pine/path/path:parse "/buffer/?name/text")))
    (is-true (pine/path/path:patternp p))
    (is (equal '(name) (pine/path/path:binders p)))
    (is (equal '((name . "scratch"))
               (pine/path/path:match p (pine/path/path:parse "/buffer/scratch/text"))))
    (is (null (pine/path/path:match p (pine/path/path:parse "/window/scratch/text"))))))

(test a-star-covers-one-name-and-two-cover-any-run
  (is (pine/path/path:match (pine/path/path:parse "/buffer/*/text")
                            (pine/path/path:parse "/buffer/scratch/text")))
  (is (null (pine/path/path:match (pine/path/path:parse "/buffer/*/text")
                                  (pine/path/path:parse "/buffer/a/b/text"))))
  (is (pine/path/path:match (pine/path/path:parse "/buffer/**/text")
                            (pine/path/path:parse "/buffer/a/b/text"))))
...
(test the-sugar-and-the-generics-mean-the-same-node
  (let ((w (pine/world/world:make-world)))
    ...
    (is (eq (pine/world/world:at w "buffer/scratch/text")
            (pine/path/place:at (pine/path/path:parse "/buffer/scratch/text") w)))
    (is (equal "hello" (pine/path/place:contents
                        (pine/path/path:parse "/buffer/scratch/text") w)))
    (setf (pine/path/place:contents (pine/path/path:parse "/buffer/scratch/text") w)
          "written through the sugar")
    (is (equal "written through the sugar"
               (pine/fs/node:contents (pine/world/world:at w "buffer/scratch/text"))))))

(test a-pattern-answers-every-node-it-covers
  (let ((w (pine/world/world:make-world)))
    (pine/world/world:place w '("buffer" "a" "text") 1)
    (pine/world/world:place w '("buffer" "b" "text") 2)
    (pine/world/world:place w '("window" "c" "text") 3)
    (is (equal '("/buffer/a/text" "/buffer/b/text")
               (mapcar #'pine/fs/node:full-name
                       (pine/path/place:matching
                        (pine/path/path:parse "/buffer/?name/text") w))))))
...
(test a-path-is-built-from-pieces-and-splices-what-it-is-given
  (let ((p (pine/path/path:path "buffer" "scratch" "text")))
    (is (equal "/buffer/scratch/text" (pine/path/path:text p)))
    (is (equal "/buffer/scratch/text/of"
               (pine/path/path:text (pine/path/path:join p "of"))))
    (is (equal "/buffer/scratch"
               (pine/path/path:text
                (pine/path/path:path (pine/path/path:parse "/buffer") "scratch")))
...
(test a-prefix-says-what-is-under-it
  (let ((buf (pine/path/path:parse "/buffer"))
        (text (pine/path/path:parse "/buffer/scratch/text")))
...
(test a-deep-wildcard-covers-any-depth-and-a-star-covers-one-name
  (let ((one (pine/path/path:parse "/buffer/*/text"))
        (deep (pine/path/path:parse "/buffer/**")))
    (is-true (pine/path/path:patternp one))
    (is-true (pine/path/path:match one (pine/path/path:parse "/buffer/scratch/text")))
    (is-false (pine/path/path:match one (pine/path/path:parse "/buffer/a/b/text"))
              "one star is one name")
    (is-true (pine/path/path:match deep (pine/path/path:parse "/buffer/a/b/text")))
    (is-true (pine/path/path:match deep (pine/path/path:parse "/buffer/a")))))

(test a-binder-says-what-it-bound
  (let ((pattern (pine/path/path:parse "/buffer/?name/text")))
    ...
    (is-false (pine/path/path:match pattern (pine/path/path:parse "/window/scratch/text")))))

(test a-path-reads-back-as-what-it-printed
  (dolist (text '("/" "/buffer" "/buffer/scratch/text" "/buffer/?name/text" "/buffer/*/text"))
