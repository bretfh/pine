...
(test a-buffer-is-parsed-by-a-task-and-not-by-whoever-is-drawing
  (with-lisp-buffer (:text "(defun f (x) x)")
    (let ((p (pine/ts/parser:parser-for (pine/edit/buffer:current))))
      ...
      (is-true (find "scratch" (pine/run/endpoint:endpoints)
                     :key #'pine/run/endpoint:name :test #'search)
...
(test a-buffer-that-is-killed-takes-its-parser-with-it
  (with-lisp-buffer (:text "(defun f (x) x)")
    ...
    (is (null (find "scratch" (pine/run/endpoint:endpoints)
                    :key #'pine/run/endpoint:name :test #'search))
...
(test a-big-buffer-is-parsed-and-painted-only-where-it-is-being-looked-at
  (with-lisp-buffer (:text "")
    (let ((b (pine/edit/buffer:current))
      ...
      (is (equal '(0 . 48) (pine/ts/parser:showing b))
          "the band is the window and a screen either side of it")
      (let ((found (pine/ts/parser:highlights b)))
        (is (< (length found) 500)
            "the walk covers the band, not six thousand lines")
        (is (every (lambda (run) (<= (first run) 48)) found)))
      (setf (pine/edit/window:scroll-of w) 3000)
      (pine/ts/parser:wait b :seconds 30)
      (is (equal '(2976 . 3048) (pine/ts/parser:showing b))
          "scrolling moves the band the parser is asked about")
      (let ((found (pine/ts/parser:highlights b)))
        (is (find-if (lambda (run) (<= 2976 (first run) 3048)) found)
            "the colours come back in the buffer's own line numbers")
        (is (find-if (lambda (run) (< (first run) 48)) found)
            "and what was walked before is still coloured, so paging does not
