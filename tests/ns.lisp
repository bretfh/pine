...
the first."
  (let ((a (pine.ns:fresh))
    ...
    (pine.ns:with-space (a)
      ...
      (pine.ui.rules:install-rules (list (list :probe-two-spaces {:min-width 11}))))
    (pine.ns:with-space (b)
      (pine.ns:raise :theme)
      (is (null (pine.ui.face:find-face :probe-face))
          "a face written in one space is in another space's table")
      (is (null (assoc ".probe-two-spaces" (pine.ui.rules:user-rules) :test #'string=))
    ...
    (pine.ns:with-space (a)
      ...
      (is (assoc ".probe-two-spaces" (pine.ui.rules:user-rules) :test #'string=)
