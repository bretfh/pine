  ...
  (handler-case
      (let* ((rt (pine.ts:make-ts-runtime))
        ...
        (let ((text (format nil "(defun f (x)~%(let ((a 1)~%(b 2))~%(+ a~%b)))~%(foo bar~%baz)")))
          ...
          (let ((got (loop for i from 0 to 6 collect (pine.ts:parse-indent ps i)))
                (want '(0 2 6 2 3 0 5)))
            (if (equal got want)
                (ok "indent: nested body + argument-alignment columns")
                (fail "indent columns: got ~s want ~s" got want))))
        (let ((ps2 (pine.ts:make-parse-state rt :commonlisp))
              (s (concatenate 'string "(defvar *x*" (string #\Newline)
                              (string #\") "a" (string #\Newline) "b"
                              (string #\") (string #\Newline) "1)")))
          (pine.ts:parse-full! ps2 s)
          (let ((got (loop for i from 0 to 3 collect (pine.ts:parse-indent ps2 i))))
            (if (equal got '(0 2 nil 2))
                (ok "indent: multiline-string interior left as-is")
                (fail "indent string interior: got ~s" got))
            (pine.ts:free-parse-state ps2)))
        (let ((ps3 (pine.ts:make-parse-state rt :commonlisp)))
          (pine.ts:parse-full! ps3 (format nil "(defcommand foo ()~%(bar))"))
          (unless (equal (list (pine.ts:parse-indent ps3 0) (pine.ts:parse-indent ps3 1))
                         '(0 2))
            (fail "indent user def-macro body"))
          (pine.ts:free-parse-state ps3))
        ...
        (let ((st (pine.buffer:load-content (format nil "(foo~%  bar)"))))
          (multiple-value-bind (new col) (pine.buffer:reindent-line st 1 2 5 4)
            (unless (and (= col 7)
                         (string= (pine.buffer:state->string new)
                                  (format nil "(foo~%     bar)")))
              (fail "reindent-line: ~s col ~s" (pine.buffer:state->string new) col))))
