...
- =state->string= is O(n) per call (625 us / 4.3 MB at 10k lines) and the
  highlight cycle calls it per edit to feed the reparse diff, which itself
  re-scans both strings. At current file sizes this is a fraction of the 5.6 ms
  cycle; making it O(edit) means the buffer describing its own edits to the
  parse state instead of diffing full strings. Worth doing when files get big;
