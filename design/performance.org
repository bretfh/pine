...
own incremental parse plus the =state->string= that feeds it. That is the next
item below, and it is now the dominant cost rather than a fraction of one.
...
- =state->string= is O(n) per call (601 us / 4.3 MB at 10k lines) and the
  highlight cycle calls it per edit to feed the reparse diff, which itself
  re-scans both strings. It is no longer a fraction of the cycle: with the
  viewport walk landing the keystroke path at 906 us, =reparse!= and the string
  it is handed are most of what is left. Making it O(edit) means the buffer
  describing its own edits to the parse state instead of diffing full strings,
  and feeding tree-sitter through a =TSInput= read callback over the fset seq so
  no contiguous copy of the file exists. Next.
