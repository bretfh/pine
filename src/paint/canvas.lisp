...
(defmethod layout:text-size ((m canvas) text font)
  (with-canvas (m)
    (cl-cairo2:select-font-face (font m) :normal :normal)
...
(defun %rect (widget)
  (values (w:left widget) (w:top widget)
          (w:width widget) (w:height widget)))
...
(defmethod layout:paint :before ((widget w:widget) (m canvas))
...
(defmethod layout:paint ((widget w:widget) (m canvas))
  (dolist (part (w:parts widget)) (layout:paint part m)))
...
(defmethod layout:paint ((widget w:label) (m canvas))
  (multiple-value-bind (x y) (%rect widget)
    (with-canvas (m)
      (rgb (%ink widget (layout:styled widget))))
    (%text m widget (w:content widget) x y)))

(defmethod layout:paint ((widget w:rule) (m canvas))
  (multiple-value-bind (x y width height) (%rect widget)
    (with-canvas (m)
      (when (rgb (%ink widget (layout:styled widget)))
...
(defmethod layout:paint ((widget w:slider) (m canvas))
  (multiple-value-bind (x y width height) (%rect widget)
    (let ((style (layout:styled widget))
...
(defmethod layout:paint ((widget w:ring) (m canvas))
  (multiple-value-bind (x y width height) (%rect widget)
    (let* ((style (layout:styled widget))
           (r (/ (- (min width height) (w:thickness widget)) 2.0))
           (cx (+ x (/ width 2.0)))
           (cy (+ y (/ height 2.0))))
      (with-canvas (m)
        ...
        (cl-cairo2:arc (float cx 1d0) (float cy 1d0) (float r 1d0)
                       (* -0.5d0 pi)
                       (+ (* -0.5d0 pi) (* 2 pi (w:fraction widget))))
        (cl-cairo2:stroke))))
  (dolist (part (w:parts widget)) (layout:paint part m)))

(defmethod layout:paint ((widget w:cells) (m canvas))
...
(defmethod layout:paint ((widget w:picture) (m canvas))
  (multiple-value-bind (x y width height) (%rect widget)
...
(defmethod layout:paint ((widget w:calendar) (m canvas))
  (multiple-value-bind (x y width) (%rect widget)
    (multiple-value-bind (cw ch) (layout:text-size m "M" (%size m widget))
      (let* ((year (w:year widget))
             (month (w:month widget))
             (today (w:day widget))
             (first-day (%weekday year month 1))
             (days (%days-in year month))
             (cell (max cw (floor width 7))))
        (with-canvas (m) (rgb (%ink widget (layout:styled widget))))
...
(defmethod layout:paint ((widget w:choice) (m canvas))
  (multiple-value-bind (x y width height) (%rect widget)
...
(defmethod layout:paint ((widget w:scroll) (m canvas))
  (multiple-value-bind (x y width height) (%rect widget)
    (with-canvas (m)
      (cl-cairo2:save)
      (cl-cairo2:rectangle (float x 1d0) (float y 1d0)
