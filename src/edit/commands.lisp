...
(defun %commands ()
  ...
  (cmd:defcommand "run-command" () (:describes "run a command by name")
    (prompt:ask "M-x " :category :command
                :then (lambda (name)
...
(defun %sources ()
  (prompt:source :command (lambda () (mapcar #'cmd:name (cmd:commands))))
  (prompt:source :buffer (lambda () (mapcar #'node:name (buffer:buffers))))
  (prompt:source :mode (lambda () (mapcar #'mode:name (mode:modes)))))
