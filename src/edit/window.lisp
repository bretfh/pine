...
(defun commands ()
  (command:defcommand "split-window-below" ()
      (:describes "two windows, one above the other")
    (split (focused) :below)
    (mapcar #'node:name (windows)))
  (command:defcommand "split-window-right" ()
      (:describes "two windows, side by side")
    (split (focused) :beside)
    (mapcar #'node:name (windows)))
  (command:defcommand "delete-window" () (:describes "close this window")
    (close-window (focused))
    (mapcar #'node:name (windows)))
  (command:defcommand "delete-other-windows" () (:describes "this window alone")
    (only (focused))
    (mapcar #'node:name (windows)))
  (command:defcommand "other-window" ()
      (:describes "move the keyboard to the next window")
    (let* ((all (windows))
           (at (or (position (focused) all) 0)))
      (focus (nth (mod (1+ at) (length all)) all))
      (node:name (focused))))
  (command:defcommand "enlarge-window" ()
      (:describes "give this window more of the room")
    (let ((win (focused)))
      (setf (weight win) (min 16 (1+ (weight win))))))
  (command:defcommand "shrink-window" ()
      (:describes "give this window less of the room")
    (let ((win (focused)))
      (setf (weight win) (max 1 (1- (weight win))))))
  (command:defcommand "balance-windows" () (:describes "every window the same size")
