(defpackage #:pine/edit/isearch
  ...
  (:export #:searching #:start #:step-search #:took #:took-all #:said
           #:needle #:forward #:wrapped #:commands))
...
(defun %look (s line col &key (forward (forward s)))
  (lines:search (d:held (doc:lines (of s))) (needle s) line col :forward forward))
...
(defun %land (s line col)
  (let ((document (of s)))
    ...
    (render:forget-spans document)
    (render:span document line col (+ col (length (needle s))) :match)))
...
(defun %shrink (s)
  (let ((had (needle s)))
    ...
    (if (zerop (length (needle s)))
        (progn (render:forget-spans (of s)) (%show s))
...
(defun commands ()
  (command:defcommand "isearch-forward" () (:describes "search as you type")
    (and (start :forward t) :searching))
  (command:defcommand "isearch-backward" () (:describes "search back as you type")
                          ...
                          (let ((n 0))
                            (loop
                              (multiple-value-bind (line col)
                                  (lines:search (d:held (doc:lines document)) from
