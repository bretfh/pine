(defpackage #:pine.text
  (:use #:cl)
  (:export #:buffer-state #:snapshot #:make-empty-state #:copy-state
           #:lines #:marks #:meta #:tick #:name #:line-count #:line-count-of
           #:point-line #:point-col #:highlights #:buffer-local
           #:state->snapshot #:state->snapshot-with-hl #:state->string
           #:split-lines #:load-content #:line-at #:region-bounds #:region-string
           #:insert-string #:insert-char #:insert-newline #:delete-char
           #:delete-region #:move-mark #:set-meta
           #:point-after-move #:move-by #:walk #:walk-line #:walk-col
           #:walk-lines #:walk-nlines #:walk-forward #:walk-back
...
(defstruct (walk (:constructor %walk (lines nlines line col)) (:copier nil))
  lines nlines line col)
...
(defun %clamped-line (state line)
  "LINE, brought inside the buffer. A region whose end ran off the buffer takes
what is there, the way a region command does when the buffer shrank under it."
  (max 0 (min line (1- (line-count-of state)))))

(defun delete-region (state start-line start-col end-line end-col)
  "Delete from START-LINE/START-COL up to END-LINE/END-COL."
  (let* ((ls (lines state))
         (start-line (%clamped-line state start-line))
         (end-line (%clamped-line state end-line))
...
(defun region-string (state start-line start-col end-line end-col)
  (let ((start-line (%clamped-line state start-line))
        (end-line (%clamped-line state end-line)))
...
pstate). With no language, (values nil pstate).

The synchronous path, for benchmarks and tools that want the cost on their own
thread: a buffer sends its parser a message and keeps going instead. EDIT is the
(line old-lines new-lines byte-delta) descriptor, when the caller has one, and
with it the tree is shifted and reused. Highlights cover NEW-STATE's :viewport
when it has one; a buffer no window has claimed gets the whole file."
...
(defun walk-word-char-p (w line col)
  (let ((s (fset:@ (walk-lines w) line)))
    (and (>= col 0) (< col (length s)) (alphanumericp (char s col)))))

(defun walk-forward (w)
  (let ((len (length (fset:@ (walk-lines w) (walk-line w)))))
    (cond ((< (walk-col w) len) (incf (walk-col w)) t)
          ((< (1+ (walk-line w)) (walk-nlines w))
           (incf (walk-line w))
           (setf (walk-col w) 0)
           t))))

(defun walk-back (w)
  (cond ((plusp (walk-col w)) (decf (walk-col w)) t)
        ((plusp (walk-line w))
         (decf (walk-line w))
         (setf (walk-col w) (length (fset:@ (walk-lines w) (walk-line w))))
         t)))

(defgeneric move-by (unit w n)
  (:documentation "Move W by N of UNIT. A motion pine does not ship -- by
paragraph, by page, by anything a mode knows the shape of -- is a method."))

(defmethod move-by ((unit (eql :char)) w n)
  (dotimes (i (abs n))
    (if (plusp n) (walk-forward w) (walk-back w))))

(defmethod move-by ((unit (eql :word)) w n)
  (dotimes (i (abs n))
    (if (plusp n)
        (progn
          (loop :while (and (not (walk-word-char-p w (walk-line w) (walk-col w)))
                            (walk-forward w)))
          (loop :while (and (walk-word-char-p w (walk-line w) (walk-col w))
                            (walk-forward w))))
        (progn
          (loop :while (and (not (walk-word-char-p w (walk-line w)
                                                   (1- (walk-col w))))
                            (walk-back w)))
          (loop :while (and (walk-word-char-p w (walk-line w) (1- (walk-col w)))
                            (walk-back w)))))))

(defmethod move-by ((unit (eql :line)) w n)
  (let ((to (max 0 (min (1- (walk-nlines w)) (+ (walk-line w) n)))))
    (setf (walk-line w) to
          (walk-col w) (min (walk-col w)
                            (length (fset:@ (walk-lines w) to))))))

(defun point-after-move (snap unit n)
  "Target (values line col) after moving point UNIT by signed N, clamped to the
buffer. Computed from SNAP's lines in one shot so a prefix count repeats
correctly."
  (let ((w (%walk (lines snap) (line-count snap)
                  (point-line snap) (point-col snap))))
    (move-by unit w n)
    (values (walk-line w) (walk-col w))))
