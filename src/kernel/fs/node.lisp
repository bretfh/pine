(defpackage #:pine/fs/node
  ...
  (:export
   ...
   #:work-out #:freshp #:cached #:saw #:in-of #:waits-of #:waitsp
...
(defvar *working* nil
  "How to hand a working-out off this thread, or nothing to run it here.")
(defvar *awaiting* nil
  "Whether this read may wait for a working-out rather than take what stands.")
...
(defmethod initialize-instance :after ((n node) &key)
  (let ((said (slot-value n 'name)))
    (unless (or (null said) (stringp said))
      (setf (slot-value n 'name) (princ-to-string said)))))
