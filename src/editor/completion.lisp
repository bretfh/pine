(defpackage #:pine.editor.completion
  (:use #:cl)
  (:export #:candidate #:candidate-actions #:candidate-annotation #:candidate-category #:candidate-source #:candidate-string #:candidate-value #:complete #:completion-popup #:completion-widget #:rank-candidates #:register-actions #:register-source #:source-table #:to-candidate))
...
;;;; same data. Selection is flagged at render (render :selection),
...
candidate's actions in the footer."
  (let* ((sel (and candidates
    ...
    (pine.ui.build:column :class "netmenu" :align :stretch
      ...
      (apply #'pine.ui.build:column :class "nm-card nm-list-card" :align :stretch :spacing 3
        (loop for c in candidates for i from 0 collect
          (pine.ui.build:row :class (if (= i selected) "nm-row sel" "nm-row")
            ...
            (pine.ui.build:icon (%category-glyph (candidate-category c)) :class "nm-sig")
