...
(defcmd "forward-char" () (pine.editor.motion:move-chars (pine.cmd:times)))
(defcmd "backward-char" () (pine.editor.motion:move-chars (- (pine.cmd:times))))
(defcmd "next-line" () (pine.editor.motion:move-lines (pine.cmd:times)))
(defcmd "previous-line" () (pine.editor.motion:move-lines (- (pine.cmd:times))))
(defcmd "forward-word" () (pine.editor.motion:move-words (pine.cmd:times)))
(defcmd "backward-word" () (pine.editor.motion:move-words (- (pine.cmd:times))))
(defcmd "kill-word" () (kill-words-cmd (pine.cmd:times)))
(defcmd "backward-kill-word" () (kill-words-cmd (- (pine.cmd:times))))
...
(defcmd "beginning-of-line" ()
  (let ((buf (pine.editor.motion:cur-buffer)) (snap (pine.editor.motion:focused-snap)))
    (when (and buf snap)
      (pine.text.buffer:put-point buf (pine.text.buffer:point-line snap) 0))))
(defcmd "end-of-line" ()
  (let ((buf (pine.editor.motion:cur-buffer)) (snap (pine.editor.motion:focused-snap)))
    (when (and buf snap)
      (let ((len (length (fset:@ (pine.text.buffer:lines snap) (pine.text.buffer:point-line snap)))))
        (pine.text.buffer:put-point buf (pine.text.buffer:point-line snap) len)))))
(defcmd "beginning-of-buffer" ()
  (let ((buf (pine.editor.motion:cur-buffer))) (when buf (pine.text.buffer:put-point buf 0 0))))
(defcmd "end-of-buffer" ()
  (let ((buf (pine.editor.motion:cur-buffer)) (snap (pine.editor.motion:focused-snap)))
    (when (and buf snap)
      (let* ((ll (1- (pine.text.buffer:line-count snap)))
             (lc (length (fset:@ (pine.text.buffer:lines snap) ll))))
        (pine.text.buffer:put-point buf ll lc)))))
...
(defcmd "execute-command" ()
  (pine.editor.minibuffer:completing-read "M-x " (pine.cmd:names)
